    ...
    return function(t, render) {
        ...
        for(let i in that.properties){
            if (that.properties[i].type.indexOf('[]') >= 0){
                ...
                if (that.properties[i].type.indexOf('{}') >= 0){
                    text+='\t\t\tfor (let i in json["'+that.properties[i].name+'"]){\n\t\t\t\tthis.' + that.properties[i].name+'.push(' + 'json["'+that.properties[i].name+'"][i] || {})\n\t\t\t}\n'
                } else if (
                ...
                if (that.properties[i].type.indexOf('{}') >= 0){
                    text+='\t\tthis.' + that.properties[i].name + ' = ' + 'json["'+that.properties[i].name+'"] || {};\n';
                } else if (
                    that.properties[i].type === 'number' ||
                    that.properties[i].type === 'boolean' ||
                    that.properties[i].type === 'string'
                    ...
                    text+='\t\tthis.' + that.properties[i].name + ' = json["'+that.properties[i].name+'"] ? new ' + that.properties[i].type + '(' + 'json["'+that.properties[i].name+'"]) : null ;\n';
                }
            }
        }
        return text
